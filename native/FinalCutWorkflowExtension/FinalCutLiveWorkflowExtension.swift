...
/// Playhead connects to the local socket published by this object.
...
public final class FinalCutLiveWorkflowExtension: NSViewController {
    ...
    public override func viewDidLoad() {
        ...
        do {
            ...
            NSLog("Playhead Final Cut live bridge failed to start at %@: %@", socket, String(describing: error))
        }
    }
    ...
    private func state() throws -> LiveState {
        guard let host, let timeline = host.timeline, let sequence = timeline.activeSequence else {
            throw NSError(domain: "Playhead", code: 1, userInfo: [NSLocalizedDescriptionKey: "no active sequence"])
        }
