...
export class FinalCutSessionAdapter implements EditorPort, LiveEditorStatePort {
  ...
  public async getCapabilities(): Promise<RuntimeCapabilities> {
    const [snapshot, mutation, live] = await Promise.all([
      this.options.snapshot?.getCapabilities(),
      this.options.mutation?.getCapabilities(),
      this.options.live?.getCapabilities(),
    ]);
    return {
      editor: {
        ...
        assetDiscovery: Boolean(snapshot?.editor.assetDiscovery),
  ...
  public async listAssets(): Promise<EditorAsset[]> {
    const provider = this.options.snapshot;
