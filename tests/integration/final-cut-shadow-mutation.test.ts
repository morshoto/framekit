...
async function createBundle(root: string): Promise<{ bundle: string; database: string }> {
  ...
  await execFile("sqlite3", [database, "CREATE TABLE ZCOLLECTION (Z_PK INTEGER PRIMARY KEY, Z_OPT INTEGER NOT NULL, ZNAME TEXT NOT NULL); INSERT INTO ZCOLLECTION VALUES (1, 1, 'Original');"]);
...
test("mutates an allowlisted collection only in a verified shadow bundle", async () => {
  ...
  try {
    ...
    const result = await mutator.renameCollection({ shadow, databaseRelativePath: "Event/CurrentVersion.fcpevent", collectionPrimaryKey: 1, name: "Revised" });
...
test("fails closed when a database path escapes its shadow bundle", async () => {
  ...
  try {
    ...
    await assert.rejects(
      mutator.renameCollection({ shadow, databaseRelativePath: "../outside.fcpevent", collectionPrimaryKey: 1, name: "Rejected" }),
...
test("fails before mutation when the Core Data schema is unsupported", async () => {
  ...
  try {
    ...
    await assert.rejects(
      mutator.renameCollection({ shadow, databaseRelativePath: "Event/CurrentVersion.fcpevent", collectionPrimaryKey: 1, name: "Rejected" }),
